...
public class Doublerep1Activity extends AppCompatActivity {

    public static String cation1;

    private static String[] cations = DoubleRep.shortenCations();
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        setContentView(R.layout.activity_doublerep1);

        //Initializing the objects
        button = (Button) findViewById(R.id.doublerep1_NextElementButton);
        lv = (ListView) findViewById(R.id.doublerep1_Element1List);
        sv = (SearchView) findViewById(R.id.doublerep1_search);
        ...
        button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Intent i=new Intent(Doublerep1Activity.this, Doublerep2Activity.class);
                startActivity(i);

                Doublerep3Activity.setArray(DoubleRep.shortenCations());

                ArrayList<String> x =new ArrayList<>(Arrays.asList(Doublerep3Activity.getArray()));

                for(int y = x.size() - 1; y >= 0; y--)
                {
                    int index = x.get(y).indexOf("-");
                    int indexLoneMetal = cation1.indexOf("-");

                    if(x.get(y).substring(0, index).equals(cation1.substring(0 , indexLoneMetal))) {
                        x.remove(y);
                    }
                }
                String [] newCations = x.toArray(new String[x.size()]);

                Doublerep3Activity.setArray(newCations);
            }
        });
...
//Sets current item selected to searchview box and makes listview invisible
        lv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            public void onItemClick(AdapterView<?> adapter, View v, int position, long id) {
                ...
                cation1 = (String) adapter.getItemAtPosition(position).toString();
                CharSequence cs = cation1;
                sv.setQuery(cs, true);
                lv.setVisibility(View.INVISIBLE);
