...
public class Doublerep2Activity extends AppCompatActivity {


    public static String anion1;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_doublerep2);


        //Initializing the objects
        button = (Button) findViewById(R.id.doublerep2_NextElementButton);
        lv = (ListView) findViewById(R.id.doublerep2_Element1List);
        sv = (SearchView) findViewById(R.id.doublerep2_search);
        ...
        button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Intent i=new Intent(Doublerep2Activity.this, Doublerep3Activity.class);
                startActivity(i);


                Doublerep4Activity.setArray(DoubleRep.shortenAnions());

                ArrayList<String> x =new ArrayList<>(Arrays.asList(Doublerep4Activity.getArray()));

                for(int y = x.size() - 1; y >= 0; y--)
                {
                    int index = x.get(y).indexOf("-");
                    int indexLoneMetal = anion1.indexOf("-");

                    if(x.get(y).substring(0, index).equals(anion1.substring(0 , indexLoneMetal))) {
                        x.remove(y);
                    }
                }
                String [] newCations = x.toArray(new String[x.size()]);

                Doublerep4Activity.setArray(newCations);
            }
        });
...
//Sets current item selected to searchview box and makes listview invisible
        lv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            public void onItemClick(AdapterView<?> adapter, View v, int position, long id) {
                ...
                anion1 = (String) adapter.getItemAtPosition(position).toString();
                CharSequence cs = anion1;
                sv.setQuery(cs, true);
                lv.setVisibility(View.INVISIBLE);
